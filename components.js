...
export function createLeftContainer() {
    ...
    // Creo un array di promesse per le richieste fetch
    const trackPromises = shuffleQueries.map(async (query) => {
        const fetchParam = {
        ...
        try {
            ...
            // restituisco l'array di elementi per la lista
            return selectTracks.map((track) => {
                return `<li class="list-group-item border-0 bg-black text-white-50 ">
          <a class="grigetto" href="./album.html?id=${track.album.id}">${track.artist.name} ${track.title}</a>
        </li>`;
            });
        } catch (error) {
            console.error("Errore durante la creazione della lista di tracce:", error);
            return []; // In caso di errore, restituisco un array vuoto
        }
    });
    ...
    Promise.all(trackPromises)
        .then((trackLists) => {
            // Unisco tutti gli array di elementi per la lista e li aggiungo all'HTML
            ulListTracks.innerHTML = trackLists.flat().join("");
        })
        .catch((error) => {
            console.error("Errore durante l'esecuzione di Promise.all:", error);
        });
}
